# Version Control and Committing Instructions

**Spec-ID:** `version-control::v1`
...
1. Require explicit user approval for operations that are not easily reversible or that alter history in a non-trivial way, plus any edits that touch files matched in `.gitignore`.
...
1. Determine the impacted workspaces and file types, then run verification steps only for the scopes that changed (for example, documentation updates may need linting, while API code changes require type checks and tests).
1. Run the commands documented in `docs/verification.md`, as required for changes.
